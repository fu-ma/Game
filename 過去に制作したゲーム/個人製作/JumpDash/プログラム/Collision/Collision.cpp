...
bool Collision::CheckRay2Sphere(const Ray &ray, const Sphere &sphere, float *distance, DirectX::XMVECTOR *inter)
{
	...
	//layの視点がsphereの外側にあり(c>0)、layがsphereから離れていく方向を指している場合(b>0),当たらない
	if (c > 0.0f && b > 0.0f)
	{
		return false;
	}
...
bool Collision::CheckSphere2Sphere(const Sphere &sphere1, const Sphere &sphere2, float r1, float r2)
{
	float X, Y, Z = 0;
	X = sphere1.center.m128_f32[0] - sphere2.center.m128_f32[0];
	Y = sphere1.center.m128_f32[1] - sphere2.center.m128_f32[1];
	Z = sphere1.center.m128_f32[2] - sphere2.center.m128_f32[2];

	//当たっていなければ
	if (!(sqrtf((X * X) + (Y * Y) + (Z * Z)) < r1 + r2))
	{
		return false;
	}
