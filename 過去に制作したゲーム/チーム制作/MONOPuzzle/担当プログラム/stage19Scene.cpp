#include "stage19Scene.h"

stage19::stage19(ISceneChanger *changer) : BaseScene(changer) {
    cam._eye = { 0,3,-5 };
    cam._target = { 0,0,5 };
    cam._up = { 0,1,0 };
    cam.UpdateViewMat();
    ...
    Raki_LoadCSV::LoadCSVFileData(map, "csv/19.csv");
    //stageLoad(map, 1);
...
void stage19::Update() {
    ...
    if (count > 180)
    {
        ...
        if (troubleCount - troubleCount2 <= speedClea)
        {
            ...
            {
                BaseScene::Get()->cleaFlag[18][0] = true;
                BaseScene::Get()->cleaFlag[18][1] = true;
                BaseScene::Get()->cleaFlag[18][2] = true;
