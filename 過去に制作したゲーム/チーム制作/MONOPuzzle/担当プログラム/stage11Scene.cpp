#include "stage11Scene.h"

stage11::stage11(ISceneChanger *changer) : BaseScene(changer) {
    cam._eye = { 0,3,-5 };
    cam._target = { 0,0,5 };
    cam._up = { 0,1,0 };
    cam.UpdateViewMat();
    ...
    Raki_LoadCSV::LoadCSVFileData(map, "csv/11.csv");
    //stageLoad(map, 1);
...
void stage11::Update() {
    ...
    if (count > 180)
    {
        ...
        if (troubleCount - troubleCount2 <= speedClea)
        {
            ...
            {
                BaseScene::Get()->cleaFlag[10][0] = true;
                BaseScene::Get()->cleaFlag[10][1] = true;
                BaseScene::Get()->cleaFlag[10][2] = true;
