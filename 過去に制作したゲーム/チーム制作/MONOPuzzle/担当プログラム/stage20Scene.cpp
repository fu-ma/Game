#include "stage20Scene.h"

stage20::stage20(ISceneChanger *changer) : BaseScene(changer) {
    cam._eye = { 0,3,-5 };
    cam._target = { 0,0,5 };
    cam._up = { 0,1,0 };
    cam.UpdateViewMat();
    ...
    Raki_LoadCSV::LoadCSVFileData(map, "csv/20.csv");
    //stageLoad(map, 1);
...
void stage20::Update() {
    ...
    if (count > 180)
    {
        ...
        if (troubleCount - troubleCount2 <= speedClea)
        {
            ...
            {
                BaseScene::Get()->cleaFlag[19][0] = true;
                BaseScene::Get()->cleaFlag[19][1] = true;
                BaseScene::Get()->cleaFlag[19][2] = true;
